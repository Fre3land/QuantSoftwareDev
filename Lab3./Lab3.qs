...
namespace Lab3 {
    ...
    operation Exercise2 (register : Qubit[]) : Unit {
        ...
        for (idx in 0 .. (numQubits / 2) - 1) {
            SWAP(register[idx], register[numQubits - 1 - i]);
        }
    }
    ...
    operation Exercise4 (register : Qubit[]) : Unit {
        let numQubits = Length(register);
        H(register[0]);
        for idx in 1 .. numQubits - 1 {
            CNOT(register[0], register[idx]);
        }
    ...
    operation Exercise7 (register : Qubit[], target: Qubit) : Unit {
        ...
        Controlled X([register[2]], target);
        Controlled X([register[1],register[2]], target);
        Controlled X([register[0],register[2]], target);
        Controlled X([register[1],register[0],register[2]], target);
    }
    ...
    operation Exercise8 (register : Qubit[]) : Unit {
        ...
        Z(register[0]);
        Controlled H([register[0]], register[1]);
        Controlled X([register[0], register[1]], register[2]);
        Z(register[2]);
    }
    ...
    operation Challenge1 (register : Qubit[]) : Unit {
        let desiredProbability = 1.0 / 3.0;
    ...
    operation Challenge3 (register : Qubit[]) : Unit {
        ...
        //normalize it so that final form is 
        // 1√6( 1/√2 |001> + |010> +  1/√2|011> -  1/√2 |101> - |110> -  1/√2|111>)
        // 1√6( 1/√2 (|001> + |011>) + |010>  -  1/√2 (|101> + |111>) - |110> )
        //final state will be 1/2(|001> + |011> -)
